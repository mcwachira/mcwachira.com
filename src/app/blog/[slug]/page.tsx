...
import Image from "next/image";
import { PostTypes } from "@/types/post";
import React, { FC } from "react"; // Import your PostTypes and FC

const iconOptions: { [key: string]: FC<any> } = {
  'Web Development': WebDevelopmentIcon,
  Business: BusinessIcon,
  'Content Creation': ContentCreationIcon,
  Tutorials: TutorialIcon,
...
export default function BlogPost({ params }: BlogPostProps) {
  const post = allPosts.find((post: PostTypes) => post?.slug === params.slug);
  const categorySlug = post?.category ? post.category.replace(/ /g, '-').toLowerCase() : '';
